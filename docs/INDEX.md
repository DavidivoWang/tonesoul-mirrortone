...
> its routing stack and authority sections below are the maintained entry logic.
> older raw registry residue has been moved out of this file into `docs/LEGACY_RAW_REGISTRY.md`.
...
The older raw registry residue that used to live in this file has been moved to:
- [LEGACY_RAW_REGISTRY.md](LEGACY_RAW_REGISTRY.md)

Use that file only when you intentionally need older inventory residue, mixed-encoding historical tables, or raw lineage lookup.
Do not use it for first-hop routing, authority, or current continuation decisions.
